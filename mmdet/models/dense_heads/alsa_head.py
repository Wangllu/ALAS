...
class ALSAHead(AnchorHead):
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    conv_cfg=self.conv_cfg,
                    norm_cfg=self.norm_cfg))
        self.alsa_cls = nn.Conv2d(
            self.feat_channels,
            self.num_anchors * self.cls_out_channels,
            3,
            padding=1)
        self.alsa_reg = nn.Conv2d(
            self.feat_channels, self.num_anchors * 4, 3, padding=1)
        self.alsa_centerness = nn.Conv2d(
    ...
    def forward_single(self, x, scale):
        ...
        cls_score = self.alsa_cls(cls_feat)
        
        bbox_pred = scale(self.alsa_reg(reg_feat)).float()
        centerness = self.alsa_centerness(reg_feat)
